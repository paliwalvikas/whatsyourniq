  ...
  class BigBasketService
    ...
    def values_filter(values, p_data)
      ...
      (0..p_data[:nutrition].length - 1).each do |a|
        ...
        p_data[:fiber] = value(val) if is_include(text ,'FIBERS') && nil_zero?(@fiber)
        p_data[:proteins] = value(val) if is_include(text ,'PROTEINS') && nil_zero?(@proteins)
        p_data[:vitA] = value(val) if (is_include(text ,'VITAMINS') && is_include(text ,'AS')) || is_include(text ,'VITAMIN-AS') && nil_zero?(@vitA)
        p_data[:vitC] = value(val) if (is_include(text ,'VITAMINS') && is_include(text ,'CS')) || is_include(text ,'VITAMIN-CS') && nil_zero?(@vitC)
        p_data[:vitD] = value(val) if (is_include(text ,'VITAMINS') && is_include(text ,'DS')) || is_include(text ,'VITAMIN-DS') && nil_zero?(@vitD)
        p_data[:vitB6] = value(val) if (is_include(text ,'VITAMINS') && is_include(text ,'B6S')) || is_include(text ,'VITAMIN-B6S') && nil_zero?(@vitB6)
        p_data[:vitB12] = value(val) if (is_include(text ,'VITAMINS') && is_include(text ,'B12S'))|| is_include(text ,'VITAMIN-B12S') && nil_zero?(@vitB12)
        p_data[:folate] = value(val) if is_include(text ,'FOLATES') && nil_zero?(@folate)
        p_data[:calcium] = value(val) if is_include(text ,'CALCIA') && nil_zero?(@calcium)
        p_data[:iron] = value(val) if is_include(text ,'IRONS') && nil_zero?(@iron)
        p_data[:iodine] = value(val) if is_include(text ,'IODINES') && nil_zero?(@iodine)
        p_data[:calories] = value(val) if is_include(text ,'CALORIES') && nil_zero?(@calories)
        p_data[:total_fat] = value(val) if is_include(text ,'TOTALS') && is_include(text ,'FATS') && nil_zero?(@total_fat)
        p_data[:saturated] = value(val) if is_include(text ,'SATURATEDS') && nil_zero?(@saturated)
        p_data[:monounsaturated] = value(val) if is_include(text ,'MONOUNSATURATEDS') && nil_zero?(@monounsaturated)
        p_data[:ployunsaturated] = value(val) if is_include(text ,'PLOYUNSATURATEDS') && nil_zero?(@ployunsaturated)
        p_data[:trans_fa] = value(val) if is_include(text ,'TRANSHES') && is_include(text ,'FATTIES') && nil_zero?(@trans_fa)
        p_data[:sodium] = value(val) if is_include(text ,'SODIA') && nil_zero?(@sodium)
        p_data[:sugar] = value(val) if is_include(text ,'SUGARS') && nil_zero?(@sugar)
        p_data[:magnesium] = value(val) if is_include(text ,'MAGNESIA') && nil_zero?(@magnesium)
        p_data[:zinc] = value(val) if is_include(text ,'ZINCS') && nil_zero?(@zinc)
        p_data[:lodine] = value(val) if is_include(text ,'LODINES') && nil_zero?(@lodine)
        p_data[:phosphorus] = value(val) if is_include(text ,'PHOSPHORUS') && nil_zero?(@phosphorus)
        p_data[:potassium] = value(val) if is_include(text ,'POTASSIA') && nil_zero?(@potassium)
        p_data[:riboflavin] = value(val) if is_include(text ,'RIBOFLAVINS') && nil_zero?(@riboflavin)
        p_data[:carbohydrate] = value(val) if is_include(text ,'CARBOHYDRATES') && nil_zero?(@carbohydrate)
        p_data[:fat] = value(val) if is_include(text ,'FATS') && nil_zero?(@fat)
        p_data[:total_sugars] = value(val) if is_include(text ,'TOTALS') && is_include(text ,'SUGARS') && nil_zero?(@total_sugars)
        p_data[:cholesterol] = value(val) if is_include(text ,'CHOLESTEROLS') && nil_zero?(@cholesterol)
      end
    end
