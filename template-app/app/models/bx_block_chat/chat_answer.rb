module BxBlockChat
  class ChatAnswer < ApplicationRecord
    ...
    def check_val?
      chat&.question&.downcase.include?('height') || chat&.question&.downcase.include?('weight')
    end
    ...
    def bmi_score
      if chat.question&.downcase.include?('weight')
        ...
        ChatAnswer.find_or_create_by(chat_id: c_id, answer: "BMI score", account_id: account.id, add_profile_id: add_profile&.id)
      end
    end
